import os
import numpy as np
import cv2 as cv


root_path = "antrenare\\"

names  = ["bart", "homer", "lisa", "marge"]

image_names = []
bboxes = []
characters = []
nb_examples = 0 #? de vzt

minx = 10000
maxx = 0
miny = 10000
maxy = 0

for name in names:
	filename_annotations = root_path + name + ".txt"
	f = open(filename_annotations)
	for line in f:
		a = line.split(os.sep)[-1]
		b = a.split(" ")
		
		image_name = root_path + name + "\\" + b[0]
		bbox = [int(b[1]),int(b[2]),int(b[3]),int(b[4])]
		character = b[5][:-1]

		#extrag dimensiunea minima si maxima a bbox-urilor din datele de antrenare
		minx = min(minx, int(b[3])-int(b[1]))
		maxx = max(maxx, int(b[3])-int(b[1]))
		miny = min(miny, int(b[4])-int(b[2]))
		maxy = max(maxy, int(b[4])-int(b[2]))
		
		image_names.append(image_name)
		bboxes.append(bbox)
		characters.append(character)


print(minx, maxx, miny, maxy) # 10 571 14 445

width_hog = 36
height_hog = 36
#compute positive examples using 36 X 36 template

for idx, img_name in enumerate(image_names):
	img = cv.imread(img_name)
	bbox = bboxes[idx]
	xmin = bbox[0]
	ymin = bbox[1]
	xmax = bbox[2]
	ymax = bbox[3]
	face = img[ymin:ymax, xmin:xmax]
	#print("original face shape:",face.shape)
	face_warped = cv.resize(face, (height_hog, width_hog))
	#print("warped face shape:", face_warped.shape)
	filename = "antrenare\\data\\exemplePozitive\\" + str(idx) + ".jpg"
	cv.imwrite(filename, face_warped)