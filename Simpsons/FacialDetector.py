...
class FacialDetector:
    ...
    def run(self):
        ...
        bart_detections = []
        bart_scores = np.array([])
        bart_file_names = np.array([])
        
        homer_detections = []
        homer_scores = np.array([])
        homer_file_names = np.array([])

        lisa_detections = []
        lisa_scores = np.array([])
        lisa_file_names = np.array([])
        
        marge_detections = []
        marge_scores = np.array([])
        marge_file_names = np.array([])


        recog: FaceRecog = FaceRecog(self.params)
        recog.train()
        ...
        low_yellow = (19, 105, 105)
        high_yellow = (50, 255, 255)
        ...
        for i in range(num_test_images):
            ...
            img_hsv = cv.imread(test_files[i])
            img_hsv = cv.cvtColor(img_hsv, cv.COLOR_BGR2HSV)
            original_mask_yellow_hsv = cv.inRange(img_hsv, low_yellow, high_yellow)
            ...
            if len(image_scores) > 0:
                ...
                for j, detection in enumerate(image_detections):
                    simpson = recog.classify(original_image[detection[1]:detection[3], detection[0]:detection[2]])
                    if simpson[0] == 0:
                        bart_detections.append(detection)
                        bart_scores = np.append(bart_scores, image_scores[j])
                        short_file_name = ntpath.basename(test_files[i])
                        bart_file_names = np.append(bart_file_names, short_file_name)
                    elif simpson[0] == 1:
                        homer_detections.append(detection)
                        homer_scores = np.append(homer_scores, image_scores[j])
                        short_file_name = ntpath.basename(test_files[i])
                        homer_file_names = np.append(homer_file_names, short_file_name)
                    elif simpson[0] == 2:
                        lisa_detections.append(detection)
                        lisa_scores = np.append(lisa_scores, image_scores[j])
                        short_file_name = ntpath.basename(test_files[i])
                        lisa_file_names = np.append(lisa_file_names, short_file_name)
                    elif simpson[0] == 3:
                        marge_detections.append(detection)
                        marge_scores = np.append(marge_scores, image_scores[j])
                        short_file_name = ntpath.basename(test_files[i])
                        marge_file_names = np.append(marge_file_names, short_file_name)
        ...
        return detections, scores, file_names, [np.array(bart_detections), np.array(homer_detections), np.array(lisa_detections), np.array(marge_detections)], [bart_scores, homer_scores, lisa_scores, marge_scores], [bart_file_names, homer_file_names, lisa_file_names, marge_file_names]
    ...
    def eval_detections(self, detections, scores, file_names, gt_path, simpson):
        ...
        plt.title(simpson + 'Average precision %.3f' % average_precision)
